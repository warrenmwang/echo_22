...
# this version has actually modified the forward function with passing data thru dropout layer...
# AND we have multiple dropout layers.
class dropout_v4_0_75_R2plus1D_18_MotionNet(nn.Module):
    def __init__(self, pretrained=True, output_channels=4):
        super(dropout_v4_0_75_R2plus1D_18_MotionNet, self).__init__()
        
        # shared R2+1d feature extractor stem
        self.r2plus1d_model = r2plus1d_18(pretrained=pretrained)
        
        # 2 3D convs before heads
        ...
        # dropout layers
        self.dropval = 0.75
        self.drop1 = nn.Dropout(self.dropval)
        self.drop2 = nn.Dropout(self.dropval)
        self.drop3 = nn.Dropout(self.dropval)
        self.drop4 = nn.Dropout(self.dropval)

        # 2 output heads (segmentation and motion)
