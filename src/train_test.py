...
def train_with_log(epoch, train_loader, model, optimizer, log_file):
    '''Exact training function as defined below, expect print statements are replaced with write to passed in argument log_file called "log.txt"'''
    model.train()
    epoch_loss = []
    ed_lv_dice = 0
    es_lv_dice = 0
    ...
    for batch_idx, batch in enumerate(train_loader, 1):
        ...
        if batch_idx % 280 == 0:
            ...
            log_file.write("ED LV: {:.3f}\n".format(ed_lv_dice / batch_idx))
            log_file.write("ES LV: {:.3f}\n".format(es_lv_dice / batch_idx))
            
            log_file.write("On a particular batch:\n")
            log_file.write(f"Deform loss: {deform_loss}\n")
            log_file.write(f"Segmentation loss: {ed_es_seg_loss}\n")
            log_file.write(f"Seg Motion loss: {segmentation_loss / video_clips.shape[0], motion_loss / video_clips.shape[0]}\n")
    
    return epoch_loss

def test_with_log(epoch, test_loader, model, optimizer, log_file):
    model.eval()
    epoch_loss = []
    ed_lv_dice = 0
    es_lv_dice = 0
    ...
    log_file.write("-" * 30 + "Validation" + "-" * 30 + '\n')
    log_file.write("\nED LV: {:.3f}\n".format(ed_lv_dice / batch_idx))
    log_file.write("ES LV: {:.3f}\n".format(es_lv_dice / batch_idx))
